...
namespace {
    ...
    void TestFormulaReferences() {
        ...
        ASSERT_EQUAL(evaluate("A1+B3"), 1);  // Cell with no text
        ASSERT_EQUAL(evaluate("A1+B1"), 1);  // Empty cell
        ASSERT_EQUAL(evaluate("A1+E4"), 1);  // Cell outside the sheet
    }
    ...
    void TestCellReferences() {
        ...
        // Reference to the cell outside the sheet
    ...
    void TestCellCircularReferences() {
        {
            ...
            ASSERT(caught);
            ASSERT_EQUAL(sheet->GetCell("E6"_pos)->GetText(), "Ready");
            ...
            ASSERT(caught);
            ASSERT_EQUAL(sheet->GetCell("C1"_pos)->GetText(), "Ready");
        }

        {
            ...
            ASSERT(caught);
            //ASSERT_EQUAL(sheet->GetCell("C2"_pos)->GetText(), "Ready");
        }
    }
}  // namespace
